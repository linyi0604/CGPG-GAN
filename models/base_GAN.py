...
class BaseGAN():
    ...
    def up_to_cuda(self, data_list):
        for data in data_list:
            for k in data:
                data[k] = data[k].to(self.device)
        return data_list

    def optimizeParameters(self, data_list):

        allLosses = {}
        self.G_input = self.data_list = self.up_to_cuda(data_list)

        self.image = data_list[-1]["image"]
        self.mask = data_list[-1]["mask"]
        n_samples = self.image.size()[0]

        # Update the discriminator
        self.optimizerD.zero_grad()

        # #1 Real data
        predRealD = self.netD(self.image, False)
        ...
        # #2 Fake data
        # batch_input = torch.cat([self.image*(1-self.mask), self.signal, noise_channel], dim=1)

        predFakeG = self.netG(self.G_input).detach()
        ...
        if self.config.lambdaGP > 0:
            allLosses["lossD_Grad"] = WGANGPGradientPenalty(self.image,
                                                            predFakeG,
                                                            self.netD,
        ...
        predFakeG = self.netG(self.G_input)
        ...
        lossSmoothL1 = self.smoothl1loss(predFakeD * self.mask, self.image * self.mask)
        lossGFake += lossSmoothL1
