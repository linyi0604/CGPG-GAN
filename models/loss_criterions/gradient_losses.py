...
def WGANGPGradientPenalty(inputs, fakes, discriminator, weight, backward=True):
    r"""
    ...
    for i in range(len(inputs)):
        ...
        interpolate_list.append(interpolates)

    decisionInterpolate = discriminator(interpolate_list, False)
    decisionInterpolate = decisionInterpolate[:, 0].sum()

    gradients = [torch.autograd.grad(outputs=decisionInterpolate,
                                    inputs=interpolate,
                                    create_graph=True, retain_graph=True)
                 for interpolate in interpolate_list]

    gradients = [gradient[0].view(batchSize, -1) for gradient in gradients]
    gradients = [(gradient * gradient).sum(dim=1).sqrt() for gradient in gradients]
    gradient_penalty = [(((gradient - 1.0)**2)).sum() * weight for gradient in gradients]
    gradient_penalty = sum(gradient_penalty)
    if backward:
