...
class GANTrainer():
    ...
    def __init__(self,
        ...
        """
        self.size_list = [(4, 4), (8, 8), (16, 16), (32, 32), (64, 64), (128, 128), (256, 256), (512, 512), (1024, 1024)]
        # Parameters
        # Training dataset
        self.path_db = pathdb
        self.path_mask = pathmask
        self.path_signal = pathsignal
    ...
    def saveCheckpoint(self, G_input, outDir, outLabel, scale, iter):
        r"""
        ...
        if self.visualisation is not None:
            ...
            ref_g = self.model.test(G_input)
            # imgSize = max(128, ref_g.size()[2])
            # self.visualisation.saveTensor(ref_g, (imgSize, imgSize),
            #                               os.path.join(outDir, outLabel + '.jpg'))

            ref_g_smooth = self.model.test(G_input, True)
            data_save = torch.cat([image_masked, ref_g, ref_g_smooth], dim=0)
            self.visualisation.saveTensor(data_save, (imgSize, imgSize),
                                          os.path.join(outDir, outLabel + '_masked_o_avg.jpg'), self.modelConfig.miniBatchSize)

    def sendToVisualization(self, G_input, scale, label=None):
        r"""
        ...
        if label is None:
            ...
            self.visualisation.publishTensors(image,
                                              (imgSize, imgSize),
                                              label + " real",
                                              env=envLabel)
        ...
        ref_g = self.model.test(G_input, False)
    ...
    def getDataset(self):
        ...
        pos = self.size_list.index(size)
        size_list = self.size_list[:pos+1]

        print("size", size)
        transform_list = [Transforms.Compose([NumpyResize(size),
                         NumpyToTensor(),
                         Transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
                          for size in size_list]
        transform_mask_signal_list = [
            Transforms.Compose([NumpyResize(size, Image.NEAREST),
            NumpyToTensor()])
            for size in size_list]

        return AttribDataset(pathdb=self.path_db,
                             pathMask=self.path_mask,
                             pathSignal=self.path_signal,
    ...
    def trainOnEpoch(self,
        ...
        for item, data in enumerate(dbLoader, 0):
            ...
            if i % self.lossIterEvaluation == 0:
                ...
                if self.visualisation is not None:
                    self.sendToVisualization(G_input, scale)
            if self.checkPointDir is not None:
                if i % self.saveIter == 0:
                    labelSave = self.modelLabel + ("_s%d_i%d" % (scale, i))
                    self.saveCheckpoint(G_input, self.checkPointDir,
                                        labelSave, scale, i)
