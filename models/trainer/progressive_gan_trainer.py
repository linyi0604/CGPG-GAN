...
class ProgressiveGANTrainer(GANTrainer):
    ...
    def __init__(self,
        ...
        GANTrainer.__init__(self, pathdb, pathmask, pathsignal, **kwargs)
    ...
    def load_model_state(self):
        # startScale = self.modelLoadPath.split("s")[-1].split("_")[0]
        # self.startScale = startScale
        self.model.load(self.modelLoadPath)
        # raise
    ...
    def train(self):
        ...
        if self.modelLoadPath is not None:
        ...
        for scale in range(self.startScale, n_scales):
            ...
            if self.checkPointDir is not None:
                ...
                label = self.modelLabel + ("_s%d_i%d" %
                                           (scale, realIter))
                self.saveCheckpoint(self.model.G_input, self.checkPointDir,
